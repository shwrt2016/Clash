...
请用 `install-mac-optimized.sh`。不要用 `install-mac.sh` 或 `verify.sh`。

## 仓库里有什么

| 文件 | 作用 |
| --- | --- |
| `install-mac-optimized.sh` | 唯一推荐入口（安装 + 验证） |
| `使用说明.txt` | 环境、依赖、步骤和常见失败 |
| `Script.js` | 旧模板，仅占位符，不要填真实密码再提交 |
| 其余 `.sh` / `dns_config.yaml` | 旧流程，新机器不要用 |
